...
public class bfs{
	...
	public void add(int item)
        ...
        public void bfs_code(int src)
        {
            ...
            {
                for(i=0;i<nodes;i++)
                    if((dist[p][i]!=0)&&(vis[i]==0))//if there is a edge between p and i and visited of i is false
                    {
                        add(i);
                        //Path path=generateCurvyPath(100,100,200,100);
                        
        Path path1 = createPath((int)circle[p].getCenterX(),(int)circle[p].getCenterY(),(int)circle[i].getCenterX(),(int)circle[i].getCenterY());
       //System.out.println("1");
        //Animation animation = createPathAnimation(path, Duration.seconds(2));
        //animation.play();
                        //System.out.println((int)circle[p].getCenterX()+" "+(int)circle[p].getCenterY()+" "+(int)circle[i].getCenterX()+" "+(int)circle[i].getCenterY());
